"""
CelebA DDPM Training with Curriculum Learning - 64x64 版本 (单GPU)

只加入 Curriculum Learning，不加入 Sparsity Curriculum
从高噪声到低噪声逐步训练

Usage:
    python train_celeba_curriculum_64.py
"""
...
def train_celeba_curriculum_64(
    device: str = "cuda:5",
    load_pth: Optional[str] = None
) -> None:
    """
    Curriculum Learning 训练 CelebA (64x64)
    ...
    # ==================== 模型 ====================
    ddpm = CurriculumDDPM(
        eps_model=NaiveUnet(3, 3, n_feat=64),
        betas=(1e-4, 0.02),
        n_T=1000
    )
    ...
    # ==================== 输出目录 ====================
    os.makedirs("./contents_curriculum_64", exist_ok=True)
    os.makedirs("./checkpoints_curriculum_64", exist_ok=True)
    ...
    for stage_idx, (t_start, t_end, stage_name) in enumerate(curriculum_stages):
        ...
        for epoch in range(epochs_per_stage[stage_idx]):
            ...
            with torch.no_grad():
                ...
                save_image(grid, f"./contents_curriculum_64/ddpm_celeba_s{stage_idx+1}_e{epoch+1}.png")

            # 保存最新模型
            torch.save(ddpm.state_dict(), "./ddpm_celeba_curriculum_64.pth")

        # Stage 结束，保存 checkpoint
        ckpt_path = f"./checkpoints_curriculum_64/ddpm_celeba_stage{stage_idx+1}.pth"
    ...
    print(f"Final model: ./ddpm_celeba_curriculum_64.pth")
    print(f"{'='*60}")


if __name__ == "__main__":
    train_celeba_curriculum_64()
